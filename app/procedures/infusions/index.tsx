...
export default function InfusionsListScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? '#000000' : '#f0f0f0' }]}>
      <Stack.Screen 
        options={{ 
          title: i18n.t('categories.infusions'),
          headerStyle: {
      ...
      >
        <View style={styles.contentContainer}>
          {infusionsTypes.map((type) => (
            <TouchableOpacity
              key={type}
              style={[styles.card, { backgroundColor: isDark ? '#1a1a1a' : '#ffffff' }]}
              onPress={() => router.push(`/procedures/infusions/${type}`)}
            >
              <View style={styles.cardHeader}>
                <Text style={[styles.title, { color: isDark ? '#ffffff' : '#000000' }]}>
                  {i18n.t(`procedures.infusions.${type}.title`)}
                </Text>
                <Ionicons 
